...
import { UserCheck, ChevronLeft, Loader2, MapPinOff } from 'lucide-react';
...
const facultyRegisterSchema = userSchema.pick({
    firstName: true,
    lastName: true,
    email: true,
    password: true,
    phoneNumber: true,
    rollNumber: true, // Used as College ID
    department: true,
    stopName: true,
}).required({
    password: true,
      ...
      <FormField
        ...
        render={({ field }) => (
          <FormItem>
            ...
            <FormControl>
              <Input placeholder="Faculty ID" {...field} />
            </FormControl>
            <FormMessage />
          </FormItem>
        )}
    ...
    <main className="flex min-h-screen flex-col items-center justify-center p-4 relative">
      ...
      <Card className="w-full max-w-md bg-background/80 backdrop-blur-lg">
        <CardHeader className="items-center text-center">
          ...
          <CardTitle className="text-2xl capitalize">Register as Faculty</CardTitle>
          <CardDescription>Request access to the bus management system.</CardDescription>
        </CardHeader>
        <CardContent>
          <RegisterForm
            role="faculty"
            schema={facultyRegisterSchema}
            defaultValues={{
              firstName: '',
              lastName: '',
              email: '',
