...
import { collection, query, where, doc, setDoc, Timestamp, updateDoc } from 'firebase/firestore';
import { useCollection, useFirestore, useMemoFirebase, useUser, useDoc } from '@/firebase';
import { UserProfile, Route, Bus, Stop, WithId, Attendance } from '@/lib/types';
...
import { ClipboardCheck, Loader2, Users, Map as MapIcon, Check, X, Edit, MapPin, Sun, Moon, Hash, Info } from 'lucide-react';
...
const BusRouteSelectionDialog = ({ userProfile, routes, buses, stops }: { userProfile: UserProfile, routes: WithId<Route>[], buses: WithId<Bus>[], stops: WithId<Stop>[] }) => {
    ...
    useEffect(() => {
        if (selectedRouteId && availableStops.length > 0 && selectedStopId && !availableStops.find(s => s.id === selectedStopId)) {
            setSelectedStopId(undefined);
        }
    }, [selectedRouteId, availableStops, selectedStopId]);

    const handleSave = async () => {
        if (!firestore || !userProfile.id || !selectedRouteId || !selectedStopId) {
...
export default function FacultyDashboard() {
  ...
  const { data: userProfile, isLoading: isProfileLoading } = useDoc<WithId<UserProfile>>(
  ...
  const { data: route, isLoading: routeLoading } = useDoc<Route>(
      useMemoFirebase(() => (userProfile?.routeId && firestore ? doc(firestore, 'routes', userProfile.routeId) : null), [userProfile?.routeId, firestore])
  );
  
  const { data: bus, isLoading: busLoading } = useDoc<WithId<Bus>>(
      useMemoFirebase(() => (userProfile?.busId && firestore ? doc(firestore, 'buses', userProfile.busId) : null), [userProfile?.busId, firestore])
  );
  
  const { data: userStop, isLoading: userStopLoading } = useDoc<Stop>(
    useMemoFirebase(() => (userProfile?.stopId && firestore ? doc(firestore, 'stops', userProfile.stopId) : null), [userProfile?.stopId, firestore])
  );

  const { data: todaysRiders, isLoading: ridersLoading } = useCollection<Attendance>(
    useMemoFirebase(() => (bus?.id && firestore ? query(
        collection(firestore, 'attendance'), 
        where('busId', '==', bus.id),
        where('attendanceDate', '==', todayStr),
        where('intention', '==', 'riding')
    ) : null), [bus?.id, firestore, todayStr])
  );
  ...
  useEffect(() => {
    ...
    if (!isProfileLoading && userProfile && userProfile.userType !== 'faculty') {
  ...
  const isLoading = isUserLoading || isProfileLoading || routeLoading || busLoading || ridersLoading || routeStopsLoading || allRoutesLoading || allBusesLoading || allStopsLoading || userStopLoading;

  if (isLoading || !userProfile || userProfile.userType !== 'faculty') {
    return (
      ...
      <main className="p-4 md:p-6 lg:p-8">
        <div className="max-w-7xl mx-auto">
          <DashboardHeader
            ...
            description="Manage student attendance and view bus information."
          />
                 ...
                 <Card className={cn(isTripActive && 'border-primary ring-1 ring-primary/20')}>
                  ...
                  <CardContent>
                    <div className="h-[450px] bg-muted rounded-lg flex items-center justify-center border shadow-inner">
                      <MapView stops={routeStops || []} bus={bus || undefined}/>
                    </div>
                  </CardContent>
                </Card>
              ...
              <div className="space-y-6">
                 <Card>
                  <CardHeader>
                    <div className='flex justify-between items-center'>
                      ...
                      {userProfile && allRoutes && allBuses && allStops && (
                  ...
                  <CardContent className="space-y-4">
                    <div>
                        <p className="text-2xl font-bold">{route?.name || 'Not Assigned'}</p>
                        <p className="text-sm text-muted-foreground">{bus?.busNumber || 'No Bus'}</p>
                    </div>
                    <div className="flex items-center gap-4 pt-2 border-t">
                        <MapPin className="w-5 h-5 text-primary" />
                        <div>
                        <p className="text-sm text-muted-foreground">Your Stop</p>
                        <p className="font-semibold">{userStop?.name || 'Not Assigned'}</p>
                        </div>
                    </div>
                  </CardContent>
                </Card>
                 <Card>
                  ...
                  <CardContent>
                    ...
                    <p className="text-xs text-muted-foreground">riding on your bus today</p>
                  </CardContent>
                </Card>
                 <Card>
                  <CardHeader>
                   ...
                   <CardContent>
                      <Link href="/faculty/attendance" passHref>
                        <Button className="w-full">
                            <ClipboardCheck className="mr-2 h-4 w-4" /> Mark Passenger Attendance
                        </Button>
                    </Link>
                  </CardContent>
                </Card>
